...
function Send()
{
	...
	var Overtime_h=document.getElementById("Overtime_h").checked;//假日加班
	
	var hour=((parseInt(End_time))+trans(E_M))-((parseInt(Start_time))+trans(S_M));//加班時數
	if(Overtime_h==true)
	{
		Overtime_class="H_overtime";
	}
	else
	{
		Overtime_class="N_overtime";
	}
	...
	if(Start_time!="" && End_time!="" && f_date!="")//#1_Start
	{
				...
				if(sumbit_later==true)//#4_Start
				{
                    ...
                    xmlhttp.onreadystatechange = function()
					{
                        if (xmlhttp.readyState == 4 && xmlhttp.status == 200)
					  {
				       ...
				       window.location.reload("../index.php/Overtime_C");
				      }
                    };
	
		var url="../index.php/Overtime_C/send"+ '/' +username+ '/' +id + '/' +start_date+ '/' +end_date+ '/' +hour+'/' +year+'/'+Overtime_class+'/'+encodeURIComponent('補單')+"/"+encodeURIComponent(reason);
		xmlhttp.open("post", url, true);
        xmlhttp.send();				
				}
				else
				{
					if(eDT.dateDiff("d",nDT)<2)
					{
						var xmlhttp = new XMLHttpRequest();//開始進行非同步傳輸(送出審核 並mail
                    xmlhttp.onreadystatechange = function()
					{
		...
		var url="../index.php/Overtime_C/send"+ '/' +username+ '/' +id + '/' +start_date+ '/' +end_date+ '/' +hour+'/' +year+'/'+Overtime_class+'/'+encodeURIComponent('加班')+"/"+encodeURIComponent(reason);
		xmlhttp.open("post", url, true);
        xmlhttp.send();				
					}
					else
